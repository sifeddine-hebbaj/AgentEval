...
async def get_diff(
    ...
    baseline_by_tc = {r.test_case_id: r for r in baseline_results}
    ...
    for new_row in new_results:
        baseline_row = baseline_by_tc.get(new_row.test_case_id)
        if baseline_row is None:
            continue
        ...
        for scorer_name, new_score in new_scores.items():
            ...
            if nv < bv - 1e-9:
                regressed.append(
                    RegressedCase(test_case_id=str(new_row.test_case_id), scorer=scorer_name, baseline_score=bv, new_score=nv)
                )
            elif nv > bv + 1e-9:
                improved.append(
                    RegressedCase(test_case_id=str(new_row.test_case_id), scorer=scorer_name, baseline_score=bv, new_score=nv)
    ...
    for scorer_name, pairs in per_scorer_pairs.items():
        ...
        sig = bootstrap_paired_delta(baseline_vals, new_vals)
        aggregate_delta[scorer_name] = sig.mean_delta
        significance[scorer_name] = SignificanceEntry(
            mean_delta=sig.mean_delta,
            ci_low=sig.ci_low,
            ci_high=sig.ci_high,
            significant=sig.significant,
            p_value_approx=sig.p_value_approx,
        )
